...
def main():
    ...
    try:
        ...
        # Recherches Google qui font apparaître chaque page (5 principales par page).
        try:
            for r in query_search_analytics(
                access_token, pages_start.isoformat(), end_date.isoformat(), ["page", "query"]
            ).get("rows", []):
                page, query = r["keys"]
                if page in pages:
            ...
            for st in pages.values():
                st["queries"] = sorted(st["queries"], key=lambda q: -q["impressions"])[:5]
        except urllib.error.HTTPError as e:
            log(f"AVERTISSEMENT : recherches par page impossibles ({e.code}).")
