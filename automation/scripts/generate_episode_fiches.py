...
5. JSON-LD dans un <script type="application/ld+json"> avec @graph contenant : WebPage (speakable) + PodcastEpisode (rattaché à la PodcastSeries {show_title}, avec author) + FAQPage (3 questions min reprises de la mega FAQ) + BreadcrumbList + Organization (name "Listenly", url https://listenly.fr, l'éditeur) + Person OU Organization "La rédaction Listenly" comme author de la WebPage et du PodcastEpisode. Le champ author DOIT être présent dans le PodcastEpisode et la WebPage.
...
8. Meta SEO complets : title de 50 à 65 caractères MAXIMUM (format court "Sujet — Podcast | Listenly", compte les caractères), meta description de 140 à 155 caractères MAXIMUM (une phrase claire sur le sujet), keywords, robots, canonical https://listenly.fr/, og:* (dont og:image={IMAGE_URL}), twitter:*. RESPECTE STRICTEMENT ces limites de longueur, c'est un critère d'audit.
...
10. AUTORITÉ (CRITÈRE GEO CRUCIAL) : dans le footer, ajouter une mention d'auteur visible : "Rédigé par La rédaction Listenly". Ajouter aussi une courte ligne d'autorité éditoriale (ex : "Listenly, annuaire de podcasts francophones").
11. RÈGLE STATISTIQUES (TRÈS IMPORTANT) : n'inclure une statistique ou un chiffre QUE si tu peux l'attribuer à une source réelle, nommée ET datée que tu connais avec certitude (ex : "selon Gallup, State of the Global Workplace 2023" ou "d'après l'INSEE 2022"). Si tu n'as pas de source fiable précise, NE DONNE AUCUN CHIFFRE : reformule la phrase de façon qualitative sans inventer de pourcentage. Mieux vaut zéro statistique qu'une statistique non sourçable. N'invente JAMAIS de chiffre. Quand tu cites une source, nomme-la explicitement dans le texte.
